...
var _pauses := []
const PAUSE_PATTERN := r"\{p=(\d+(\.\d+)?)\}"  # Corrected regex pattern with raw string
...
func _find_pauses(source_string: String) -> void:
	...
	for _result in _found_pauses:
		var _tag_string = _result.get_string()
		var _tag_position = _adjust_position(_result.get_start(), source_string)
...
func _adjust_position(pos: int, source_string: String) -> int:
	...
	# Regex for BBCode tags
	var _bbcode_i_regex := RegEx.new()
	var _bbcode_e_regex := RegEx.new()

	_bbcode_i_regex.compile(r"\[(?!\/)(.*?)\]")
	_bbcode_e_regex.compile(r"\[\/(.*?)\]")
